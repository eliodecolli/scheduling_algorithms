class RoundRobin:
    ...
    def processData(self, no_of_processes, **kwargs):
        process_data = []
        for i in range(no_of_processes):
            temporary = []
            process_id = i#int(input("Enter Process ID: "))
            arrival_time = kwargs.__getitem__('arrival_time')[i]#int(input(f"Enter Arrival Time for Process {process_id}: "))
            burst_time = kwargs.__getitem__('burst_time')[i]#int(input(f"Enter Burst Time for Process {process_id}: "))
...
if __name__ == "__main__":
    ...
    rr = RoundRobin()
    rr.processData(3, burst_time = [5,4,10], time_slice=3, arrival_time= [0,2,3] )